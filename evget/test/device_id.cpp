...
TEST(DeviceId, DeterministicUuidSameNames) {
    const boost::uuids::uuid ns = boost::uuids::string_generator{}("6ba7b810-9dad-11d1-80b4-00c04fd430c8");
    const std::string name{R"(\\?\HID#VID_046D&X)"};

    evget::DeviceId<std::string> first{};
    evget::DeviceId<std::string> second{};

    ASSERT_EQ(first.UuidDeterministic(name, ns), second.UuidDeterministic(name, ns));
}
...
TEST(DeviceId, DeterministicRepeatedCalls) {
    ...
    const std::string first = device_ids.UuidDeterministic(name, ns);
    ASSERT_EQ(first, device_ids.UuidDeterministic(name, ns));
}
...
TEST(DeviceId, DeterministicDistinctNames) {
    ...
    for (const std::string& name : {std::string{"A"}, std::string{"B"}, std::string{"C"}, std::string{"D"}}) {
        uuids.insert(device_ids.UuidDeterministic(name, ns));
    }
